...
vector<double> MetricLearningDiag::error(const set<size_t> &indexSet, int Ksample) const{
    if(indexSet.empty())
        return vector<double>(0);
    vector<double> err(Ksample,0);
    vector<size_t> indexVec;
    indexVec.reserve(indexSet.size());
    for (size_t index : indexSet)
        indexVec.push_back(index);
    #pragma omp parallel 
    {
        #pragma omp for reduction(vec_double_plus :err)
        for (size_t I=0;I<indexVec.size();++I ){
            size_t index=indexVec[I];
            ...
            for (size_t k = 0, size = distance.size(); k < (size_t) Ksample && k < size; ++k){
            ...
            if (tripletLR[i].first <= lambda){
                if (tripletLR[i].second){
                    ...
                    if (L_Active[i]){
                        const VectorXd& a = X.row(get<0>(tripletIndex[i])) - X.row(get<2>(tripletIndex[i]));
                        const VectorXd& b = X.row(get<0>(tripletIndex[i])) - X.row(get<1>(tripletIndex[i]));
                        H_L_Active -= a.cwiseProduct(a)-b.cwiseProduct(b), --count_L_Active, L_Active[i]=false;
                    }

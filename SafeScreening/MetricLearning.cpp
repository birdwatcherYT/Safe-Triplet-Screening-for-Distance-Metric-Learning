...
double MetricLearning::run(const Bound &bound, double lambda_prev, double lambda, double gamma, double alpha, double eps, int freq, unsigned loopMax, ostream &os) {
    ...
    for (unsigned loop = 0; loop < loopMax; ++loop) {
        ...
        double primal = loss + 0.5 * lambda * M2;
                ...
                case DGB_SEMIDEF:{
                    ...
                    for (size_t i = 0; i < length; ++i) {
                        ...
                        if (dot[i] <= 1-gamma || 1 <= dot[i]){
                            ...
                            const MatrixXd& Hijl =  a*a.transpose()-b*b.transpose();
                            if (dot[i] <= 1 - gamma) {
                                if (!existLoose(1 - gamma, Hijl, M, r2)){
                                    ++count_L, screening[i]=true;
                                    continue;
                                }
                            } else {
                                if (!existLoose(1, Hijl, M, r2)){
                                    ++count_R, screening[i]=true;;
                                    continue;
                                }
                            }
                    ...
                    for (size_t i = 0; i < length; ++i) {
                        ...
                        if (dot[i] <= 1-gamma || 1 <= dot[i]){
                            ...
                            if (dot[i] <= 1 - gamma) {
                                if (QmHijl <= 0)
                                    continue;
                                double alpha= -sqrt( (Hijl2*Qm2-QmHijl*QmHijl)/(Qm2*r2) );
                                double beta = - QmHijl/Qm2;
                                if (HijlQ-(beta*QmHijl+Hijl2)/alpha <= 1-gamma){
                                    ++count_L, screening[i]=true;
                                    continue;
                                }
                            } else {
                                ...
                                double alpha= sqrt( (Hijl2*Qm2-QmHijl*QmHijl)/(Qm2*r2) );
                                double beta = - QmHijl/Qm2;
                                if (HijlQ-(beta*QmHijl+Hijl2)/alpha >= 1){
                                    ++count_R, screening[i]=true;
                                    continue;
                                }
                            }
